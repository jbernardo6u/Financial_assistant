...
from .models import FinancialIndicator, FinancialDocument, Company
...
class FinancialDocumentAdmin(admin.ModelAdmin):
    list_display = ('company','year', 'revenue', 'net_income', 'operating_expenses', 'r_and_d_costs', 'shares_outstanding',
...
class FinancialIndicatorAdmin(admin.ModelAdmin):
    list_display = ('company', 'year', 'document', 'gross_profit_margin', 'net_profit_margin', 'operating_profit_margin', 'profit_growth_rate',
                    'earnings_per_share', 'debt_ratio', 'current_ratio', 'cash_ratio', 'operating_cash_flow',
                    'free_cash_flow', 'capital_expenditure_ratio')
